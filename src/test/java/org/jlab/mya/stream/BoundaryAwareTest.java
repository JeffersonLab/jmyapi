...
public class BoundaryAwareTest {
    ...
    public void testAddNeither() throws Exception {
        ...
        events.add(new FloatEvent(TimeUtil.toLocalDT("2019-02-01T00:00:00"), EventCode.NETWORK_DISCONNECTION, 1));
        events.add(new FloatEvent(TimeUtil.toLocalDT("2019-03-01T00:00:00"), EventCode.NETWORK_DISCONNECTION, 2));
        events.add(new FloatEvent(TimeUtil.toLocalDT("2019-04-01T00:00:00"), EventCode.NETWORK_DISCONNECTION, 3));
        events.add(new FloatEvent(TimeUtil.toLocalDT("2019-05-01T00:00:00"), EventCode.NETWORK_DISCONNECTION, 4));
        ...
        List<FloatEvent> eventList = new ArrayList<>();
        try (EventStream<FloatEvent> stream = new ListStream<>(events, FloatEvent.class)) {
            try (BoundaryAwareStream<FloatEvent> boundaryStream = new BoundaryAwareStream<>(stream, begin, end, priorPoint, true, FloatEvent.class)) {
                FloatEvent event;
                while ((event = boundaryStream.read()) != null) {
                    eventList.add(event);
                    System.out.println(event.toString());
