...
public class FloatGraphicalSampleStreamTest {
    ...
    public void testBasicUsage() throws IOException {
        ...
        events.add(new FloatEvent(10000000, EventCode.CHANNELS_PRIOR_DATA_MOVED_OFFLINE, 17)); // First, included
        ...
        events.add(new FloatEvent(14000000, EventCode.UPDATE, 15)); // lttb, max, prior to disconnect, included
        events.add(new FloatEvent(15000000, EventCode.NETWORK_DISCONNECTION, 0)); // disconnect, last, included
        ...
        try (EventStream<FloatEvent> stream = new ListStream<>(events, FloatEvent.class)) {
            try (FloatGraphicalSampleStream<FloatEvent> sampleStream = new FloatGraphicalSampleStream<>(stream, 1, 6, FloatEvent.class)) {
                FloatEvent e;
    ...
    public void testBasicUsage2() throws IOException {
        // This is an artificial scenario since EventConde.CHANNELS_... indicates that there are no prior data points,
        ...
        events.add(new FloatEvent(10000000, EventCode.CHANNELS_PRIOR_DATA_MOVED_OFFLINE, 17)); // First data event, included
        events.add(new FloatEvent(11000000, EventCode.UPDATE, -1)); // min, included
        events.add(new FloatEvent(12000000, EventCode.UPDATE, 11)); // skipped
        events.add(new FloatEvent(13000000, EventCode.UPDATE, 10)); // skipped
        events.add(new FloatEvent(14000000, EventCode.UPDATE, 15)); // lttb, max, prior to disconnect, included
