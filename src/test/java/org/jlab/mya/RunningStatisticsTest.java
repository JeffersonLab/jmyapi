...
public class RunningStatisticsTest {
  ...
  public void testReset() {
    ...
    // The mean should have been 2 before the reset and null after.  Check both to make sure things
  ...
  public void testGetSigma() {
    ...
    // mean = 2, var = sum ( w_i * (x_i - mean)^2 ) / sum ( w_i ) = [ 1*(2-2)^2 + 2*(1-2)^2 +
    // 2*(3-2)^2 ] / 5 = 4/5. sigma = sqrt(var) = sqrt(4/5)
  ...
  public void testGetRms() {
    ...
    // mean = 2, var = sum ( w_i * (x_i - mean)^2 ) / sum ( w_i ) = [ 1*(2-2)^2 + 2*(1-2)^2 +
    // 2*(3-2)^2 ] / 5 = 4/5. RMS = sqrt(var + mean^2) = sqrt(4/5 +4)
