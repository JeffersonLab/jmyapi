...
public class ApplicationLevelSamplingTest {
  ...
  public void testSimpleEventBinSampler() throws Exception {
    ...
    long expSize =
        10; // Not sure it will always be exact, might be +/- 1 in some combinations of count and
    // limit
    List<FloatEvent> eventList = new ArrayList<>();
    try (EventStream<FloatEvent> stream = nexus.openEventStream(metadata, begin, end)) {
      FloatSimpleSampleStream<FloatEvent> sampleStream =
          new FloatSimpleSampleStream<>(stream, limit, count, FloatEvent.class);
      FloatEvent event;
  ...
  public void testAnalyzeThenSimpleEventBinSampler() throws Exception {
    ...
    start = System.currentTimeMillis();
    try (final EventStream<FloatEvent> stream = nexus.openEventStream(metadata, begin, end);
        ...
        final FloatAnalysisStream analysisStream =
            new FloatAnalysisStream(boundaryStream, eventStatsMap);
        final FloatSimpleSampleStream<AnalyzedFloatEvent> sampleStream =
            new FloatSimpleSampleStream<>(
                analysisStream, limit, count, AnalyzedFloatEvent.class); ) {
  ...
  public void testGraphicalSampler() throws Exception {
    ...
    // Impossible to know how many data points will be generated a priori since every disconnect
    // will be represented.
    // The expected size gets complicated to predict.  Number of actual bins is a complicated
    // calculation based on determining
    // the smallest bin size that produces no more than numBins-2.  Then each bin can produce min,
    // max, and largest triangle three bucket (lttb) point
    // and also return an unlimited number of non-update events plus surrounding update events.  A
    // good rule of thumb for number of points returned is
    ...
    try (EventStream<FloatEvent> stream = nexus.openEventStream(metadata, begin, end)) {
      try (FloatGraphicalSampleStream<FloatEvent> sampleStream =
          new FloatGraphicalSampleStream<>(stream, numBins, count, FloatEvent.class)) {
        FloatEvent event;
        while ((event = sampleStream.read()) != null) {
          eventList.add(event);
    ...
    // Since we know ahead of time there are 20 points of data, start + end + (10 bins with
    // min,max,lttb points) + zero non-update events = between 12 and 20
