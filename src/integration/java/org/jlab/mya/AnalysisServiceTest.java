...
public class AnalysisServiceTest {
  ...
  public void testCalculateRunningStatistics() throws Exception {
    ...
    Instant begin =
        LocalDateTime.of(2016, Month.OCTOBER, 1, 0, 0, 0)
     ...
     */
    System.out.println("\nMARCAA.BDL");
    //        compareStats(nexus, begin, end, "MARCAA.BDL", 0, 3.47227e+06, 3.45818e+06, 140699,
    // 9.8968e+13);
  ...
  private void compareStats(
    ...
    try (final EventStream<FloatEvent> stream = nexus.openEventStream(metadata, begin, end);
        final EventStream<FloatEvent> boundaryStream =
    ...
    Assert.assertEquals(
        mean,
        result.getMean(),
        Math.abs(percDiff * mean)); // difference within a percentage of the expected value
    ...
    // This test is known to fail as of now.  I believe this is due to a difference in time
    // representation
