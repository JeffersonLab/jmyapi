...
public class PerformanceTest {
  ...
  public void doNestedStreamsTest() throws SQLException, IOException {
    ...
    System.out.println(
        "Memory used at this instant (MB): "
            + String.format("%,.2f", (stopBytes) / 1024.0 / 1024.0));
   ...
   *
   * <p>Looks like streaming is nearly always best if count > 16,000. If less than perhaps 4096 then
   * transferring results all at once back is generally slightly faster. If users wanted to be
   * clever and they knew result size (do a count query first) then they could choose a strategy
   * dynamically. Likely this is unnecessary though since streaming (the default in jmyapi) is
   * always the best in terms of minimal memory usage and is almost as good with small datasets in
   * terms of speed as other options and actually better than rest with large datasets presumably
   * since users can begin processing the data nearly immediately (less latency) instead of waiting
   * for a big chunk (or all of it) to arrive.
   *
