...
/**
 * Wraps an IntEventStream and provides LabelEnumEvents by assigning enumeration labels to integer
 * values.
 *
 * @author slominskir
 */
public class LabeledEnumStream extends WrappedStream<LabeledEnumEvent, IntEvent> {
  ...
  public LabeledEnumStream(EventStream<IntEvent> stream, List<ExtraInfo> enumLabelList) {
    ...
    this.enumLabelList =
        new ArrayList<>(enumLabelList); // We make a copy because later we may modify the list
