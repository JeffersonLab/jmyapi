 ...
 * that.
 *
 * <p>A channel remains open until closed so generally you'll want to use a try-with-resources
 * approach to using this class or it's subclasses. Be advised that generally an open EventStream
 * holds database resources open and should be closed as quickly as possible (generally the first
 * stream in a set of possibly nested streams is a DatabaseSourceStream, which holds a database
 * connection).
 *
 * <p>This class is abstract and concrete implementations provide Mya event type specific streams.
 *
 * @author slominskir
...
public abstract class EventStream<T extends Event> implements Channel {
   ...
   *
   * <p>In Java, Generics are erased at runtime so if you want to know what type something is you
   * must pass it as an argument. That's why type info is specified twice in EventStream: once for
   * the compiler and once for the runtime. All so you can call the method getType() at runtime!
   *
   * @return The type
   */
