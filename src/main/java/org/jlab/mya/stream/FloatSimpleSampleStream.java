...
/**
 * Wraps an EventStream and provides FloatEvents that are down-sampled as they stream by using a
 * simple algorithm.
 *
 * <p>This stream reads the full dataset from the database and returns a subset (performs
 * application layer filtering).
 *
 * <p>This algorithm bins by count, not by date interval like many other sampling algorithms. There
 ...
 * periods.
 *
 * <p>Another feature of this algorithm is it streams over the entire dataset once instead of
 * issuing n-queries (n = # of bins). There are pros and cons to this as well. This algorithm will
 * generally perform better than issuing n-queries would if there are only a few points per bin.
 *
 * <p>Note: Users must figure out number of events per bin threshold in which to use n-queries
 * instead.
 *
...
public class FloatSimpleSampleStream<T extends FloatEvent> extends WrappedStream<T, T> {
  ...
  public FloatSimpleSampleStream(EventStream<T> stream, long limit, long count, Class<T> type) {
    ...
    if (count > limit && limit > 0) {
      ...
      this.fractional = BigDecimal.valueOf((count % limit) / (double) limit);
    } else {
      this.binSize = 1;
  ...
  public T read() throws IOException {
    ...
    if (old.setScale(0, RoundingMode.DOWN)
            .compareTo(fractionalCounter.setScale(0, RoundingMode.DOWN))
        == 0) {
