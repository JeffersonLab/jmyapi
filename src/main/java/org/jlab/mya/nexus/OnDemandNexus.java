...
public class OnDemandNexus extends DataNexus {
  ...
  static {
    ...
    try (InputStream is =
        DataNexus.class.getClassLoader().getResourceAsStream("credentials.properties")) {
      if (is == null) {
  ...
  Connection getConnection(String host) throws SQLException {
    ...
    options.put(
        "useCompression",
        "true"); // 1.5 Million records streams in 8 seconds with compression and 16 seconds without

    options.put(
        "noAccessToProcedureBodies",
        "true"); // Required to use stored procedures from limited access user
    ...
    try {
      ...
      throw new SQLException("Unable to obtain connection to host: " + host + "; port: " + port, e);
    }
  }
}
