...
class IntervalService extends QueryService {
        ...
        try (ResultSet rs = stmt.executeQuery()) {
          if (rs.next()) {
            ...
            throw new SQLException(
                "Unable to count records in table_" + params.getMetadata().getId());
          }
        }
      }
    }
   ...
   * a generic stream which will need to be cast to obtain values (values may be primitives so a
   * generic getValue() method returning a generic would not work unless you want to accept
   * primitive autoboxing performance cost).
   *
  ...
  public <T extends Event> EventStream<T> openEventStream(IntervalQueryParams<T> params)
      throws SQLException {
  ...
  /**
   * Open a stream to multi-string-valued events associated with the specified IntervalQueryParams.
   *
   * <p>Generally you'll want to use try-with-resources around a call to this method to ensure you
   * close the stream properly.
   *
   ...
   * @throws SQLException If unable to query the database
   */
  private MultiStringEventStream openMultiStringStream(IntervalQueryParams params)
      throws SQLException {
    InternalIntervalParams iip = openStream(params);
  ...
  private InternalIntervalParams openStream(IntervalQueryParams params) throws SQLException {
    ...
    int fetchSize = 0; // 0 means fetch all records into memory
