...
class PointQueryParams<T extends Event> extends QueryParams<T> {
  ...
  /**
   * Create a new PointQueryParams with the default behavior of searching for the event occurring at
   * a time less than or equal to the timestamp provided and including all event types.
   *
  ...
  /**
   * Return true if the event search looks for the last event before the point-in-time or false if
   * the event search looks for the first event after the point in time.
   *
  ...
  /**
   * Return true if the event search looks for the event exactly at the point-in-time or false if
