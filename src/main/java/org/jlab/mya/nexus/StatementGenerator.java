...
class StatementGenerator {
  /**
   * Get the SQL list of event code numbers that match to data updates. E.g., "where column in
   * [some_list]".
   *
  ...
  private static String getDataEventListString() {
    return "("
        + String.join(
            ",",
            EventCode.getDataEventCodes().stream()
   ...
   *
   * <p>Note: clever implementations may be caching / pooling statements.
   *
   ...
   * @throws SQLException If unable to prepare a statement
   */
  PreparedStatement getExtraInfoStatement(Connection con, String type, Instant end)
      throws SQLException {
   ...
   * for events.
   *
   * <p>The fetchSize parameter should be used carefully. Use value Integer.MIN_VALUE to instruct
   * MySQL driver to stream results. This is the safest in terms of ensuring you don't run out of
   * memory, but may not be the fastest method. If set to a positive number 'n' AND
   * com.mysql.jdbc.Connection.setUseCursorFetch(true) then a server-side cursor (temporary table)
   * is used to spoon feed the results 'n' at a time. This is likely the worst option as results
   * don't start arriving on the client until after the server has fully filled the temporary table.
   * This option puts more stress on the server too. Finally, the third option is enabled if
   * fetchSize is not Integer.MIN_VALUE and setUserCursorFetch(true) is also not set. In this case
   * the fetchSize value is ignored (as if set to zero) and the MySQL driver will fetch the entire
   * ResultSet all at once into memory. This may be the fastest method (for small data), but may
   * cause your JVM to run out of memory (with large data).
   *
   * @param con The connection the statement belongs to
   ...
   * @throws SQLException If unable to prepare a statement
   */
  PreparedStatement getEventIntervalStatement(
      Connection con, IntervalQueryParams params, int fetchSize) throws SQLException {
   ...
   * @throws SQLException If unable to prepare a statement
   */
  PreparedStatement getCountStatement(Connection con, IntervalQueryParams params)
      throws SQLException {
  ...
  /**
   * Return a prepared statement for the given connection and parameters to query for a single event
   * at a given point in time. Depending on parameters the query may search for the last event
   * before (or equal) the point-in-time or the first event after (or equal) the point in time.
   *
   * @param con The connection the statement belongs to
   * @param params The query parameters associated with the statement (notably metadata id)
   * @return The PreparedStatement
   * @throws SQLException If unable to prepare a statement
   */
  PreparedStatement getEventPointStatement(Connection con, PointQueryParams params)
      throws SQLException {
    String equal = "";
    String sign = ">";
    String sort = "asc";
