...
public class LabeledEnumEvent extends IntEvent {
  ...
  /**
   * Return a String representation of this IntEvent using a timestamp with zero fractional seconds
   * displayed.
   *
  ...
  /**
   * Return a String representation of this IntEvent using a timestamp with the specified fractional
   * second precision displayed. Note: using a value of 6 (microseconds) is generally the max
   * precision (A precision up to 9, nanoseconds, is supported, but rounding errors prevent proper
   * conversion).
   *
  ...
  /**
   * Find the enum label given the int value and the history labels. NOTE: the enumLabelList may be
   * modified as old enum labels that are not used are dropped from the list as an optimization for
   * future calls, which are assumed to be made in asc order.
   *
   * @param iEvent The IntEvent to be labeled
   * @param enumLabelList The list of historical enum labels; which may be modified when the call
   *     completes
   * @return A LabeledEnumEvent
   */
  public static LabeledEnumEvent findLabelFromHistory(
      IntEvent iEvent, List<ExtraInfo> enumLabelList) {
    ...
    if (iEvent != null) {
      ...
      if (enumLabelList != null) {
        ...
        for (int i = 0; i < enumLabelList.size(); i++) {
          ...
          boolean infoLessThanOrEqual =
              info.getTimestamp().isBefore(timestamp) || info.getTimestamp().equals(timestamp);
        ...
        // This optimization just says if we've already passed old historical enum labels discard
        // them so they aren't considered in future events
